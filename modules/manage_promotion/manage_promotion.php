...
class manage_promotion extends Aruna_Controller
{
	protected $csrf;
	...
	public function category()
	{
		...
		if ($this->input->post('step') && $this->input->post('step') == 'post')
		{
			...
			else
			{
				...
				$this->db->sql_insert($data, 'ml_promotion_category');
	...
	public function editcategory($id)
	{
		...
		$res = $this->db->sql_prepare("select id, name from ml_promotion_category where id = :id");
		...
		if ($this->input->post('step') && $this->input->post('step') == 'post')
		{
			...
			else
			{
				...
				$this->db->sql_update($data, 'ml_promotion_category', ['id' => $id]);

				echo json_encode(['status' => 'success', 'url' => site_url('manage_promotion/category')]);
				exit;
			}
		}
		...
		if ($this->input->post('step') && $this->input->post('step') == 'post')
		{
			...
			else
			{
				if ( ! empty($_FILES['thumbnail']))
				{
					...
					$s_parentfolder = './contents/userfiles/promotion/'.$dir_yearmonth.'/';
					$s_subfolder = './contents/userfiles/promotion/'.$dir_yearmonth.'/'.$subdir_date.'/';

					// For database only without dot and slash at the front folder
					$x_folder = 'contents/userfiles/promotion/'.$dir_yearmonth.'/'.$subdir_date.'/';
	...
	public function editpost($id)
	{
		...
		if ($this->input->post('step') && $this->input->post('step') == 'post')
		{
			...
			else
			{
				...
				if ($status_upload == 0)
				{
					$this->db->sql_update($data, 'ml_promotion_article', ['id' => $row['id']]);

					echo json_encode(['status' => 'success', 'message' => 'Post Edited', 'url' => site_url('manage_promotion')]);
					exit;
				}
			}
		}
	...
	public function editseo($article_id = 0)
	{
		...
		if ($this->input->post('step') && $this->input->post('step') == 'post')
		{
			...
			else
			{
				...
				if ($status_upload == 0)
				{
					$this->db->sql_update($data, 'ml_metatag_article', ['article_id' => $article_id, 'type' => 'promotion']);
	...
	public function setcustomfield($id)
	{
		...
		if ($this->input->post('step') && $this->input->post('step') == 'post')
		{
			if ($this->input->post('text') !== null)
			{
				...
				$this->db->sql_update($data_vars, 'ml_promotion_article', ['id' => $row['id']]);
	...
	{
		set_title('Layout Promotion Settings');

		load_extend_view('default', ['header_dash_page', 'footer_dash_page']);

		$res_layout = $this->db->sql_prepare("select * from ml_layout where page = :page and section = :section");
		$bindParam_layout = $this->db->sql_bindParam(['page' => 'promotion', 'section' => 'main_content'], $res_layout);
	...
	public function getListPosts()
	{
		...
		$res_getTotal = $this->db->sql_prepare("select count(*) as num from ml_promotion_article where (cid like :cid and title like :title)");
		...
		$res = $this->db->sql_prepare("select * from ml_promotion_article where (cid like :cid and title like :title) order by id desc limit $this->offset, $this->num_per_page");
	...
	public function getDetailMetatag($article_id)
	{
		...
		$bindParam = $this->db->sql_bindParam(['article_id' => $article_id, 'type' => 'promotion'], $res);
		while ($row = $this->db->sql_fetch_single($bindParam))
		{
	...
	public function deletepost($id)
	{
		...
		if ($check)
		{
			$res = $this->db->sql_prepare("select thumb_l, thumb_s from ml_promotion_article where id = :id");
			$bindParam = $this->db->sql_bindParam(['id' => $id], $res);
			$row = $this->db->sql_fetch_single($bindParam);
			...
			$this->db->sql_delete("ml_promotion_article", ['id' => $id]);
	...
	{
		$res = $this->db->sql_select("select * from ml_promotion_category order by id desc");
	...
	public function deletecategory($id)
	{
		...
		$check = $this->db->num_rows("ml_promotion_category", "", ['id' => $id]);

		if ($check)
		{
			$this->db->sql_delete("ml_promotion_category", ['id' => $id]);
			$this->output->set_content_type('application/json', 'utf-8')
					 ->set_header('Access-Control-Allow-Origin: '.site_url())
					 ->set_output(json_encode(['status' => 'success'], JSON_PRETTY_PRINT))
					 ->_display();
	...
	{
		$res = $this->db->sql_prepare("select * from ml_promotion_article where uri = :uri");

<?php
	...
	section_content(breadcrumb(['Pages' => site_url('manage/pages'), 'Edit Page' => '', $name_page => '']));
	...
	<div <div class="container-fluid" id="ar-app-form">
		<div class="bg-white arv3-pc-content p-3 p-md-4 rounded shadow-sm" id="ar-form-submit">
			...
			<form action="'.site_url('manage/editpage/'.$row['uri']).'" method="post" enctype="multipart/form-data" @submit="submit" ref="formHTML" button-block="false" button-rounded-pill="false" font-size-large="false">');
		...
		foreach ($values as $key => $value) 
		{
			...
			elseif ($value['type'] == 'text')
			{
				section_content('
								<div class="col-12 form-group mb-3">
									<label class="form-label">'.$label.'</label>
									<input type="'.$value['type'].'" name="get_'.$keys.'['.$key.']" class="form-control" value="'.$value['content'].'">
								</div>');
			}
		}

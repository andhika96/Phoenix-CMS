<?php
	...
	<div class="container py-3 py-md-5" id="ar-app-listdata-article">
		<div class="ar-fetch-listdata" id="ar-data" data-url="'.site_url('promotion/getListPosts').'">
			<div class="pb-5">
				<div class="row">
					<div class="col-md-4 mb-3 mb-md-0">
						<div class="h2 fw-bold">Our Promotion</div> <p>find the latest promotion from us '.get_layout('promotion', 'is_hide_category').'</p>
					</div>';

			if (get_layout('promotion', 'is_hide_category') == 1)
			{
				echo '
					<div class="col-md-4 offset-md-4">
						<label class="form-label">Search</label>
						<div class="input-group ph-search-container">
							<input type="text" class="form-control font-size-inherit" placeholder="Search by title" aria-label="Search by title" aria-describedby="search-promotion" v-model="getSearch" @keyup="searchData">
							<span class="input-group-text" id="search-promotion"><i class="fas fa-search font-size-inherit"></i></span>
						</div>
					</div>';
			}
			else
				...
				<div v-else>
					...
					<transition-group name="custom-classes-transition" enter-active-class="animate__animated animate__fadeIn animate__faster" tag="div" class="row ph-grid-view">
						<div class="col-md-6 col-xl-4" v-for="(info, index) in getData" v-bind:key="info.id">
							...
							<div class="ph-promotion">
								<div>
									<div class="thumbnail">
										<a :href="\''.site_url('promotion/\'+info.uri+\'').'\'" class="stretched-link">
									...
									<div class="details">
										<div class="wrapper">
											<h4 class="title text-truncate mb-3">
												<a :href="\''.site_url('promotion/\'+info.uri+\'').'\'" class="stretched-link"><span>{{ info.title }}</span></a>
											</h4> 
											
											<div class="my-3">{{ info.get_date }}</div> 
											<a :href="\''.site_url('promotion/\'+info.uri+\'').'\'" class="more_listing text-mg-red">Read More</a>
										</div>
									</div>
								</div>
							</div>

...
class promotion extends Aruna_Controller
{
	protected $offset;
	...
	public function __construct() 
	{
		...
		// Check active page
		check_active_page('promotion', ['style_class_name' => 'm-5']);
	}

	public function index()
	{
		set_title(t('Promotion'));

		set_meta(site_url('promotion'), 'Promotion', 'Get the latest promotion', NULL);

		if (get_layout('promotion', 'view_type') == 'grid')
		{
			$data['layout_view'] = $this->grid_view();
		}
		elseif (get_layout('promotion', 'view_type') == 'grid_box')
		{
			$data['layout_view'] = $this->grid_box_view();
		}
		elseif (get_layout('promotion', 'view_type') == 'list')
		{
	...
	{
		$res_category = $this->db->sql_select("select * from ml_promotion_category order by id asc");
	...
	public function detail($uri = '')
	{
		...
		// Prpromotion from Automatic conversion of false to array is deprecated
		$row = ($row !== FALSE) ? $row : [];
		...
		auto_set_meta('promotion', $row['id']);
	...
	{
		$res_getTotal = $this->db->sql_prepare("select count(id) as num from ml_promotion_article where (cid like :cid and title like :title) and status = :status and schedule_pub <= :time");
		...
		$res = $this->db->sql_prepare("select * from ml_promotion_article where (cid like :cid and title like :title) and status = :status and schedule_pub <= :time order by schedule_pub desc limit $this->offset, $this->num_per_page");
		...
		while ($row = $this->db->sql_fetch_single($bindParam))
		{
			...
			$row['category']	= get_category($row['cid'], 'promotion');
			$row['get_date']	= ( ! empty($row['schedule_pub'])) ? get_date($row['schedule_pub'], 'date') : get_date($row['created'], 'date');

<?php
	...
	if (get_module_actived('event') == 0)
	{
		...
		<div class="bg-warning p-3 rounded mb-3 text-dark bg-opacity-25">
			<i class="fad fa-exclamation-triangle fa-lg fa-fw me-1"></i> '.t('The {1} page has been disabled, but you can still manage content from this page', ucfirst(get_module('event', 'name'))).'
		</div>');
	}
...
.preview2 img
{
	...
	<div class="container-fluid px-0">
		<div class="bg-white arv3-pc-content p-3 p-md-4 rounded shadow-sm" id="ar-app-listdata">
			<div class="h5 pb-3 pb-md-4 mb-3 border-bottom d-flex justify-content-between align-items-center"><div><i class="fad fa-calendar-star fa-fw me-2"></i> '.t('Manage Event').'</div> <div><span class="float-md-right font-size-normal"><a href="'.site_url('manage_event/addpost').'">Add New Post</a></div></div>
			
			<div class="row mb-2 ar-fetch-listdata" id="ar-data" data-url="'.site_url('manage_event/getListPosts').'">
				<div class="col-md-4 mb-3 mb-md-0">
					<div class="form-group">
						<label class="form-label">Search Event by Title</label>
						...
						<table class="table table-striped table-hover">
							...
							<tbody is="transition-group" name="custom-classes-transition" enter-active-class="animate__animated animate__fadeIn animate__faster">
								<tr v-for="(info, index) in getData" v-bind:key="info.id">
									...
									<td class="align-middle text-nowrap">
										<a :href="\''.site_url('manage_event/editpost/\'+info.id+\'').'\'" class="btn btn-light font-size-inherit"><i class="fas fa-edit"></i></a>
										<a href="javascript:void(0)" v-on:click="deleteData(getData, index, info.id); show = !show" class="ar-alert-bootbox btn btn-light text-danger font-size-inherit ms-1" v-bind:data-url="\''.site_url('manage_event/deletepost/').'\'"><i class="fas fa-trash"></i></a>
									</td>
								</tr>
							</tbody>
						</table>

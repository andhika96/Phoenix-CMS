<?php
	...
	section_content(breadcrumb(['Admin Panel' => site_url('awesome_admin'), 'Admin' => '']));

	section_content('
	<div class="mb-5">
		<div class="arv6-box bg-white p-3 p-md-4 rounded shadow-sm" id="ar-listuser">
			<div class="h5 pb-3 pb-md-4 mb-4 mb-md-5 mb-xl-4 border-bottom"><i class="fad fa-user-secret fa-fw me-2"></i> Manage Admin Account</div>
			<div class="sk-notice-toast position-relative" aria-live="polite" aria-atomic="true"></div>

			<div class="ar-fetch" id="ar-listuser-load" data-fetch-user="'.site_url('awesome_admin/getAdmin').'" data-url-user="'.site_url('awesome_admin/getAdminData?id=').'">
				<div class="row mb-2">
					<div class="col-md-4 mb-2 mb-md-0">
						<div class="form-group">
							<label>Search Admin by Fullname or Username</label>
				...
				<div v-else class="ar-data-load" style="display: none">
					...
					<div v-else>
						<form action="'.site_url('awesome_admin/admin').'" method="post" enctype="multipart/form-data" button-block="false" data-reset="true" @submit="submit" ref="formHTML">
										...
										<tr>
											<th scope="col" style="width: 5%" class="text-center">
												<div class="custom-control custom-checkbox">
													<input type="checkbox" class="custom-control-input" id="clickSelectAll" v-on:click="clickSelectAll">
													<label class="custom-control-label" for="clickSelectAll"></label>
												</div>
											</th>
									...
									<tbody is="transition-group" name="custom-classes-transition" enter-active-class="animate__animated animate__fadeIn animate__faster">
										<tr class="text-nowrap" v-for="(info, index) in getData" v-bind:key="info.id">
											<td class="align-middle text-center">
												<div class="custom-control custom-checkbox">
													<input type="checkbox" name="getSelected[]" class="custom-control-input checkids" v-on:click="clickCheckbox(info.id)" v-bind:id="\'user_\'+info.id" v-bind:value="info.id">
													<label class="custom-control-label" v-bind:for="\'user_\'+info.id"></label>
												</div>
											</td>
											...
											<td class="align-middle">
												...
												<span class="d-block mt-1">{{ info.fullname }}</span>
											</td>
							...
							<div class="row mt-2">
								<div class="col-md-3 pr-md-0 mb-3 mb-md-0">
									<select name="changestatus" class="custom-select">
								...
								<div class="col-md-3 pr-md-0 mb-3 mb-md-0">
									<select name="changerole" class="custom-select">
										<option value="">Change Role</option>');
									
								while ($row = $db->sql_fetch_array($res_role)) 
								{
									section_content('
		...
		section_content('
								...
								<div class="col">
									...
									<input type="submit" class="btn btn-bnight-blue btn-malika font-size-inherit" value="Submit">
								</div>
							</div>
						</form>
					</div>

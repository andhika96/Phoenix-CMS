...
class event extends Aruna_Controller
{
	...
	{
		$res_getTotal = $this->db->sql_prepare("select count(id) as num from ml_event_article where (cid like :cid and title like :title) and schedule_pub <= :time");
		...
		$res = $this->db->sql_prepare("select * from ml_event_article where (cid like :cid and title like :title) and schedule_pub <= :time order by schedule_pub desc limit $this->offset, $this->num_per_page");

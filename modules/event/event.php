...
class event extends Aruna_Controller
{
	protected $offset;
	...
	public function __construct() 
	{
		...
		// Check active page
		check_active_page('event', ['style_class_name' => 'm-5']);
	}
	...
	public function index()
	{
		...
		elseif (get_layout('event', 'view_type') == 'grid_box')
		{
			$data['layout_view'] = $this->grid_box_view();
		}
		elseif (get_layout('event', 'view_type') == 'list')
		{
	...
	{
		$res_category = $this->db->sql_select("select * from ml_event_category order by id asc");
	...
	{
		$res_getTotal = $this->db->sql_prepare("select count(id) as num from ml_event_article where (cid like :cid and title like :title) and status = :status and schedule_pub <= :time");
		...
		$res = $this->db->sql_prepare("select * from ml_event_article where (cid like :cid and title like :title) and status = :status and schedule_pub <= :time order by schedule_pub desc limit $this->offset, $this->num_per_page");
		...
		while ($row = $this->db->sql_fetch_single($bindParam))
		{
			...
			$row['category']	= get_category($row['cid'], 'event');
			$row['get_date']	= ( ! empty($row['schedule_pub'])) ? get_date($row['schedule_pub'], 'date') : get_date($row['created'], 'date');

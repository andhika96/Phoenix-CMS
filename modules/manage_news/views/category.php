<?php
	...
	section_content(breadcrumb([t('Manage News') => site_url('manage_news'), t('List of Category') => '']));
		...
		<div class="row" id="ar-app-listdata">
			<div class="col-md-6 mb-4 mb-md-0">
				<div class="arv6-box bg-white arv3-pc-content p-3 py-md-4 px-md-3 rounded shadow-sm">
					...
					<div v-else class="ar-data-load" style="display: none">
						<transition-group name="ar-fade" class="list-group list-group-flush ar-fetch-listdata-wopage" tag="ul" data-url="'.site_url('manage_news/getListCategories').'">
							<li class="list-group-item list-group-item-action" v-for="(info, index) in getDataWOPage" v-bind:key="info.id" v-if="info.id">
								{{ info.name }} <span class="float-end"><a :href="\''.site_url('manage_news/editcategory/\'+info.id+\'').'\'">Edit</a> - <a href="javascript:void(0)" v-on:click="deleteData(getDataWOPage, index, info.id); showData = !showData" class="ar-alert-bootbox" v-bind:data-url="\''.site_url('manage_news/deletecategory/').'\'">Delete</a></span>
							</li>
						</transition-group>
					</div>
				</div>
			...
			<div class="col-md-6" id="ar-form-submit">
				<div class="arv6-box bg-white arv3-pc-content p-3 py-md-4 px-md-3 rounded shadow-sm">
					...
					<form action="'.site_url('manage_news/category').'" method="post" @submit="submit" button-block="false" font-size-large="false" button-rounded-pill="false" with-list-wopage="true" with-list-slideshow-page="false" with-list-coverimage-page="false" ref="formHTML">

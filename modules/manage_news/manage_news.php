...
class manage_news extends Aruna_Controller
{
	protected $csrf;
	...
	public function editcategory($id)
	{
		...
		$res = $this->db->sql_prepare("select id, name from ml_news_category where id = :id");
		...
		if ($this->input->post('step') && $this->input->post('step') == 'post')
		{
			...
			else
			{
				...
				$this->db->sql_update($data, 'ml_news_category', ['id' => $id]);

				echo json_encode(['status' => 'success', 'url' => site_url('manage_news/category')]);
				exit;
			}
		}
		...
		if ($this->input->post('step') && $this->input->post('step') == 'post')
		{
			...
			else
			{
				if ( ! empty($_FILES['thumbnail']))
				{
					...
					$s_parentfolder = './contents/userfiles/news/'.$dir_yearmonth.'/';
					$s_subfolder = './contents/userfiles/news/'.$dir_yearmonth.'/'.$subdir_date.'/';

					// For database only without dot and slash at the front folder
					$x_folder = 'contents/userfiles/news/'.$dir_yearmonth.'/'.$subdir_date.'/';
				...
				if ($status_upload == 0)
				{
					$this->db->sql_insert($data, 'ml_news_article');
					...
					echo json_encode(['status' => 'success', 'url' => site_url('manage_news/editpost/'.$getLatestID)]);
					exit;
				}
			}
		}
	...
	public function editpost($id)
	{
		...
		register_js([
			'<script src="https://cdn.jsdelivr.net/momentjs/latest/moment.min.js"></script>',
			'<script src="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.min.js"></script>',
			'<script src="'.base_url('assets/plugins/ckeditor5/build/ckeditor.js?v=0.0.4').'"></script>',
			'<script src="'.base_url('assets/plugins/ckfinder/ckfinder.js').'"></script>',
	...
	public function layout()
	{
		...
		$bindParam_layout = $this->db->sql_bindParam(['page' => 'news', 'section' => 'main_content'], $res_layout);
	...
	public function getListPosts()
	{
		...
		$res_getTotal = $this->db->sql_prepare("select count(*) as num from ml_news_article where (cid like :cid and title like :title)");
		...
		$res = $this->db->sql_prepare("select * from ml_news_article where (cid like :cid and title like :title) order by id desc limit $this->offset, $this->num_per_page");
	...
	public function deletepost($id)
	{
		...
		if ($check)
		{
			$res = $this->db->sql_prepare("select thumb_l, thumb_s from ml_news_article where id = :id");
			$bindParam = $this->db->sql_bindParam(['id' => $id], $res);
			$row = $this->db->sql_fetch_single($bindParam);
			...
			$this->db->sql_delete("ml_news_article", ['id' => $id]);
	...
	{
		$res = $this->db->sql_select("select * from ml_news_category order by id desc");
	...
	public function deletecategory($id)
	{
		...
		$check = $this->db->num_rows("ml_news_category", "", ['id' => $id]);
